...
export default function ShowToServerPage(_: ShowPageProps) {
    ...
    <div className="min-h-screen bg-white text-zinc-900">
      <header className="sticky top-0 z-10 border-b border-zinc-100 bg-white/90 backdrop-blur">
        <div className="mx-auto flex w-full max-w-3xl items-center justify-between px-4 py-3 sm:px-6 sm:py-4">
          <Link className="p-1 text-sm text-zinc-600" href={`/r/${restaurant.id}/cart`}>
            ← Back
          </Link>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {item.variations.length > 0 && (
                      <div className="mt-1 text-xs text-zinc-500 sm:mt-2">
                        {item.variations
                          .map((variation) => variation.nameNative)
                          .join(", ")}
                      </div>
                    )}
                  </div>
                  <div className="text-base font-semibold sm:text-lg">x{item.quantity}</div>
                </div>
              </div>
            ))
          )}
